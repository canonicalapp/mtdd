...
- **Type Safety**: Full TypeScript implementation with ts-proto generated types
- **Code Generation**: Automatic TypeScript type generation from `.proto` files using ts-proto
...
- `npm run proto:generate` - Generate TypeScript types from `.proto` files (auto-detects protoc or Docker)
...
This project uses **ts-proto** for type-safe Protocol Buffer code generation. This approach provides:

- ✅ **Full TypeScript type safety** - Compile-time type checking
- ✅ **Cross-platform support** - Works on Windows ARM64, Linux, macOS
- ✅ **No native binaries** - Pure TypeScript generation
- ✅ **Modern TypeScript** - Idiomatic, clean generated code
- ✅ **Works with Node.js v25+** - No compatibility issues

#### Directory Structure

- **`proto/`**: Source of truth - all `.proto` files live here (standard practice)
- **`src/generated/`**: Auto-generated TypeScript code from ts-proto

#### Code Generation Process

**Prerequisites:**

- `protoc` compiler (or Docker for cross-platform support)
- `ts-proto` npm package (installed as dev dependency)

**Steps:**

1. **Define API**: Edit `proto/db.proto` with your service contract
2. **Generate types**: Run `npm run proto:generate`
   - Uses local `protoc` if available
   - Falls back to Docker if `protoc` is not installed
...
- Uses local `protoc` if available (fastest)
- Falls back to Docker if `protoc` is not installed
- Shows helpful error message with installation instructions if neither is available
...
│       └── db.ts                 # Generated TypeScript types and service definitions
├── scripts/
│   └── generate-proto.sh         # Cross-platform proto generation script
